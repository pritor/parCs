...
////////////////////////////////////////////////////////////////////////////
//расчет уравнения теплопроводности по блокам и потокам. Используем j по x и i по y при каждом расчете, принцип с четвертьюсумммой остается тем же
////////////////////////////////////////////////////////////////////////////
...
void printGrid(double *array, int size) {                  //функция распечатывающая сетку
...
////////////////////////////////////////////////////////////////////////////
    ...
    cudaMallocHost(&arr, realsize * sizeof(double));                 //выделение pinned памяти для массивов
    cudaMallocHost(&arr_new,
                   realsize * sizeof(double));             //к inned памяти имеют доступ как процессор, так и gpu
...
////////////////////////////////////////////////////////////////////////////
    ...
    double *error;
    cudaMallocHost(&error, sizeof(double));         //выделение pinned памяти для ошибки
    *error = 1.0;
...
////////////////////////////////////////////////////////////////////////////
    ...
    dim3 blockDim((size > 32) ? threads / 32 : threads, (size > 32) ? threads / 32 : threads);
    dim3 gridDim((size > 32) ? blocks*32 : blocks, (size > 32) ? blocks*32 : blocks);
    ...
    for (; (k < iternum) && (*error > accuracy);) {
        if (isGraphCreated) {
            ...
            for (size_t i = 0; i < 50; i++) {
                heat_equation<<<gridDim, blockDim, 0, stream>>>(Matrix, MatrixNew,
                                                                size);            //при такой схеме swap не нужен
